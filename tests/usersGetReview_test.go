...
func TestUsersGetReviewHappyPath(t *testing.T) {
	...
	resp, err := http.Get(server.URL + "/users/getReview?user_id=a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a12")
	if err != nil {
		t.Fatalf("request failed: %v", err)
	}
	defer resp.Body.Close()

	expectedResponse, err := os.ReadFile("static/usersGetReviewResponse.json")
	if err != nil {
		t.Fatalf("failed to read json file: %v", err)
	}
